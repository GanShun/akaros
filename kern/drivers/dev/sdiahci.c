...
#define	iprintd(...)	if(prid)	printd(__VA_ARGS__)
#define	aprintd(...)	if(datapi)	printd(__VA_ARGS__)
...
static void
sdierror(struct cmdbuf *cb, char *fmt, ...)
{
...
static void
aesleep(struct aportm *pm, Asleep *a, int ms)
{
	ERRSTACK(2);
	if(waserror())
		return;
...
static int
ahciwait(struct aportc *c, int ms)
{
	Asleep as;
	struct aport *p;
...
static void
listsetup(struct aportc *pc, int flags)
{
	struct alist *list;
...
static int
setfeatures(struct aportc *pc, unsigned char f)
{
	unsigned char *c;
...
static int
setudmamode(struct aportc *pc, unsigned char f)
{
	unsigned char *c;
...
static int
ahciflushcache(struct aportc *pc)
{
	unsigned char *c;
...
static int
ahciidentify0(struct aportc *pc, void *id, int atapi)
{
	unsigned char *c;
	struct aprdt *p;
...
static int64_t
ahciidentify(struct aportc *pc, uint16_t *id)
{
	int i, sig;
	int64_t s;
	struct aportm *pm;
...
resetdisk(Drive *d)
{
	...
	printd("ahci: %s: resetdisk: %s → %s\n", (d->unit? d->unit->sdperm.name: NULL),
...
static void
isctlrjabbering(struct ctlr *c, uint32_t cause)
{
	uint32_t now;
...
static void
iainterrupt(struct hw_trapframe *unused_hw_trapframe, void *a)
{
	int i;
	uint32_t cause, mask;
	struct ctlr *c;
...
static struct alist*
ahcibuildpkt(struct aportm *pm, struct sdreq *r, void *data, int n)
{
	int fill, len;
	unsigned char *c;
	struct alist *l;
...
static int
iariopkt(struct sdreq *r, Drive *d)
{
	ERRSTACK(2);
 ...
 */
static int
iaahcimode(struct pci_device *p)
{
	printd("iaahcimode: %#x %#x %#x\n", pcidev_read8(p, 0x91), pcidev_read8(p, 92),
	       pcidev_read8(p, 93));
...
static int
newctlr(struct ctlr *ctlr, struct sdev *sdev, int nunit)
{
	int i, n;
	Drive *drive;
	...
	printk("#S/sd%c: %s: %#p %s, %d ports, irq %d\n", sdev->idno,
	       Tname(ctlr), ctlr->physio, descmode[i], nunit, ctlr->pci->irqline);
	/* map the drives -- they don't all need to be enabled. */
	n = 0;
	ctlr->rawdrive = kzmalloc(NCtlrdrv * sizeof(Drive), 0);
...
static int
iarctl(struct sdunit *u, char *p, int l)
{
	char buf[32];
	char *e, *op;
	struct aport *o;
	...
	p = seprintf(p, e, "reg\ttask %#lx cmd %#lx serr %#lx %s ci %#lx "
...
static int
iawctl(struct sdunit *u, struct cmdbuf *cmd)
{
	ERRSTACK(2);
	char **f;
	struct ctlr *c;
...
static char *
portr(char *p, char *e, unsigned int x)
{
	int i, a;
...
static char*
iartopctl(struct sdev *sdev, char *p, char *e)
{
	uint32_t cap;
	char pr[25];
	Ahba *hba;
...
#define has(x, str) if(cap & (x)) p = seprintf(p, e, "%s ", (str))
...
static int
iawtopctl(struct sdev *sdev, struct cmdbuf *cmd)
{
	int *v;
	char **f;
