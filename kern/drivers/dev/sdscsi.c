...
scsiverify(struct sdunit* unit)
{
	...
	if((r = kzmalloc(sizeof(struct sdreq), 0)) == NULL)
		return 0;
	if((inquiry = kzmalloc(sizeof(unit->inquiry), MEM_WAIT)) == NULL){
		kfree(r);
		return 0;
	}
	r->unit = unit;
...
int
scsiexec(struct sdunit* unit, int write, uint8_t* cmd, int clen,
	 void* data,
	 int* dlen)
{
	struct sdreq *r;
...
int32_t
scsibio(struct sdunit* unit, int lun, int write, void* data, int32_t nb,
	uint64_t bno)
{
	struct sdreq *r;
	int32_t rlen;
