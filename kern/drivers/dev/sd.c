...
/* In Plan 9, this array is auto-generated. That's almost certainly not necessary;
...
void
sdaddpart(struct sdunit* unit, char* name, uint64_t start, uint64_t end)
{
	struct sdpart *pp;
	int i, partno;
	...
	if (end > unit->sectors)
		error(EINVAL, "%s: end %d > number of sectors %d", __func__, end, unit->sectors);
	pp = &unit->part[partno];
	pp->start = start;
	pp->end = end;
...
sddelpart(struct sdunit* unit, char* name)
{
	...
	/* no permission checking in akaros at present.
...
sdadddevs(struct sdev *sdev)
{
	...
	for(; sdev; sdev=next){
		...
		sdev->unit = (struct sdunit**)kzmalloc(sdev->nunit * sizeof(struct sdunit *),
						       0);
		...
		for(i=0; i<ARRAY_SIZE(devs); i++){
			if(devs[j = (id+i)%ARRAY_SIZE(devs)] == NULL){
...
// 	char buf[2];
//
// 	snprintf(buf, sizeof buf, "%c", sdev->idno);
// 	unconfigure(buf);
// }

void
sdaddallconfs(void (*addconf)(struct sdunit *))
{
	int i, u;
	struct sdev *sdev;
...
static int
sdgen(struct chan* c, char* d, struct dirtab* dir, int j, int s,
      struct dir* dp)
{
	struct qid q = {};
	int64_t l;
...
static int32_t
sdstat(struct chan* c, uint8_t* db, int32_t n)
{
...
sdopen(struct chan* c, int omode)
{
	...
	case Qraw:
		...
		if(test_and_set_bit(0,
		                    (volatile unsigned long *)&unit->rawinuse) != 0){
			c->flag &= ~COPEN;
			kref_put(&sdev->r);
			error(EBUSY, "In use");
		}
...
static int32_t
sdbio(struct chan* c, int write, char* a, int32_t len, int64_t off)
{
	ERRSTACK(2);
	int nchange;
	uint8_t *b;
...
static int32_t
sdrio(struct sdreq* r, void* a, int32_t n)
{
	ERRSTACK(2);
	void *data;
	...
	data = NULL;
	if(n){
		if((data = kzmalloc(n, MEM_WAIT)) == NULL)
 ...
 */
int
sdsetsense(struct sdreq *r, int status, int key, int asc, int ascq)
{
	int len;
	struct sdunit *unit;
...
int
sdmodesense(struct sdreq *r, uint8_t *cmd, void *info, int ilen)
{
	int len;
	uint8_t *data;
...
int
sdfakescsi(struct sdreq *r, void *info, int ilen)
{
	uint8_t *cmd, *p;
	uint64_t len;
	struct sdunit *unit;
...
static int32_t
sdwstat(struct chan* c, uint8_t* dp, int32_t n)
{
	ERRSTACK(2);
	struct dir *d;
	struct sdpart *pp;
...
static int
sdconfig(int on, char* spec, struct devconf* cf)
{
	if(on)
