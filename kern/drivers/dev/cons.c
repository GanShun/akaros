 ...
 */
int
kbdcr2nl(struct queue* ignored_queue, int ch)
{
	char *next;
 ...
 */
int
kbdputc(struct queue* unused_queue, int ch)
{
	int i, n;
	char buf[3];
...
int
consreadnum(uint32_t off, char *buf, uint32_t n, uint32_t val, int size)
{
	char tmp[64];
...
int
consreadstr(uint32_t off, char *buf, uint32_t n, char *str)
{
	int size;
...
consread(struct chan *c, void *buf, long n, int64_t off)
{
	...
	case Qpgrpid:
		return consreadnum((uint32_t)offset, buf, n, current->pgrp->pgrpid, NUMSIZE);

	case Qpid:
		return consreadnum((uint32_t)offset, buf, n, current->pid, NUMSIZE);

	case Qppid:
		return consreadnum((uint32_t)offset, buf, n, current->ppid, NUMSIZE);
