...
int alloc_intr_pages(void)
{
	...
	for (int i = 0; i < vm->nr_gpcs; i++) {
		...
		/* Set APIC ID. */
		//((uint32_t *)gpcis[i].vapic_addr)[0x20/4] = i << 24;
		((uint32_t *)gpcis[i].vapic_addr)[0x20/4] = i;
		//((uint32_t *)gpcis[i].apic_addr)[0x20/4] = i;
		/* Set APIC VERSION. */
		((uint32_t *)gpcis[i].vapic_addr)[0x30/4] = 0x01060015;
		//((uint32_t *)gpcis[i].apic_addr)[0x30/4] = i;
		/* Set LOGICAL APIC ID. */
...
int main(int argc, char **argv)
{
	...
	*m = madt;
	...
	/* Set maxcpus to the number of cores we're giving the guest. If there's
	 * only one cpu, turn on nosmp.
	 * */
	len = snprintf(cmdlinep, cmdlinesz,
